...
### 6. Variable Definitions Files

Variables in the Terraform Cloud workspace and variables provided through the command line always overwrite variables with the same key from files ending in `*.tfvars`.

In Terraform Cloud, if your workspace contains a `terraform.tfvars` file and there are workspace variables, the workspace variables will override the ones in the `terraform.tfvars` file. 

Note that in Terraform Enterprise, if your workspace contains a `terraform.tfvars` file, it will be overwritten by the workspace variables.

Refer to this [section](https://www.terraform.io/language/values/variables#variable-definition-precedence) on Variable Precedence for more details.
